...
/- Traditional Lean 4 style -/
def addArraysTraditional (a b : Array Int) : Option (Array Int) :=
  if h : a.size = b.size then
    some (Array.zipWith a b (· + ·))
  else
    none

theorem addArraysTraditional_spec (a b : Array Int) (h : a.size = b.size) :
  match addArraysTraditional a b with
  | some res => res.size = a.size ∧ 
                ∀ i : Fin res.size, res[i] = a[i.val]'(by simp [res.size_eq]; exact i.isLt) + 
                                              b[i.val]'(by rw [←h]; simp [res.size_eq]; exact i.isLt)
  | none => False :=
by sorry
