...
theorem findMinBy_spec {α : Type} (a : Array α) (lt : α → α → Bool)
  ...
  ⦃⌜a.size > 0⌝⦄
    findMinBy a lt
  ⦃⇓ r => ⌜∃ i : Fin a.size, a[i] = r ∧ (∀ i : Fin a.size, ¬lt a[i] r)⌝⦄ := by
  unfold findMinBy
  mvcgen
  case inv => exact ⇓ (minVal, xs) =>
...
def findMinOfTwo (a b : Array Int) : StateT Nat Id Int := do
  let minA ← findMin a
  let minB ← findMin b
...
theorem findMinWithLog_spec (a : Array Int) :
  ⦃⌜a.size > 0⌝, logs => ⌜logs = []⌝⦄
    findMinWithLog a
  ⦃⇓ r, logs => ⌜existsIn a r ∧ isLowerBound a r ∧ logs.length > 0⌝⦄ := by
  unfold findMinWithLog
  mvcgen
  case inv => exact ⇓ (minVal, xs), logs =>
    ⌜logs.length > 0 ∧
    existsIn (a.take xs.rpref.length) minVal ∧
    isLowerBound (a.take xs.rpref.length) minVal⌝
  all_goals sorry
