...
theorem addArrays_hoare (a b : Array Int) :
  ...
  ⦃ fun res => spred(
      match res with
      | Array.some arr => a.size = b.size → (arr.size = a.size ∧ ∀ i : Fin arr.size, arr[i] = a[i.val]'(by simp [arr.size_eq]; exact i.isLt) + b[i.val]'(by rw [arr.size_eq]; simp; exact i.isLt))
      | none => a.size ≠ b.size
    ) ⦄ := by
  mspec
  intro _
